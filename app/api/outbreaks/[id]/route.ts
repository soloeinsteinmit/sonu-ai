...
export async function DELETE(
  ...
  try {
    ...
    if (id.startsWith('csv-')) {
      ...
      const dataDir = process.env.PREDICTIONS_DIR || (process.env.VERCEL ? '/tmp' : path.join(process.cwd(), 'server'));
      const filePath = path.join(dataDir, 'predictions.csv');

      if (!fs.existsSync(filePath)) {
        return NextResponse.json({ error: 'No outbreak data found' }, { status: 404 });
      }
      ...
      const canDeleteCsv = isWithinDeletionRange(userLat, userLon, outbreakLat, outbreakLon, 5);

      if (!canDeleteCsv) {
      return NextResponse.json(
        { 
          error: 'You must be within 5km of the outbreak location to delete it',
            distance: calculateDistance(userLat, userLon, outbreakLat, outbreakLon)
        },
        { status: 403 }
      );
    }
      ...
      console.log(`CSV outbreak ${id} deleted by user at ${userLat}, ${userLon}. Reason: ${reason || 'No reason provided'}`);

      return NextResponse.json({ message: 'Outbreak successfully deleted' }, { status: 200 });
    }
